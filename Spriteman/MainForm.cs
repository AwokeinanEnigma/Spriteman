	...
	public partial class MainForm : Form
	{
			...
			using (Graphics graphics = Graphics.FromImage(this.image))
			{
				...
				for (int i = 0; i < this.image.Height; i++)
				{
					for (int j = 0; j < this.image.Width; j++)
					{
						...
						int num = (int)this.indices[i * this.image.Width + j];
		...
		private void SaveSpriteSheet(string filename)
		{
			...
			foreach (object obj in this.spriteList.Items)
			{
				...
				nbtCompound3.Add(new NbtIntArray("crd", new int[]
				{
					sprite.Coords.X,
					sprite.Coords.Y
				}));
				nbtCompound3.Add(new NbtIntArray("bnd", new int[]
				{
					sprite.Bounds.Width,
					sprite.Bounds.Height
				}));
				nbtCompound3.Add(new NbtIntArray("org", new int[]
				{
					sprite.Origin.X,
					sprite.Origin.Y
				}));
				nbtCompound3.Add(new NbtInt("frm", sprite.Frames));
		...
		public void LoadENUS(string filename) {
			...
			NbtCompound rootTag = new NbtFile(filename).RootTag;
			foreach (var thing in rootTag.Tags) {
